...
class InsertBookRepository(val app: Application, var insertBookModel: BookModelInsert) {

    var insertBookData  = MutableLiveData<MessageModel>()

    @WorkerThread
    fun callWebServiceInsert() {
        ...
        val call = service.insetBook(insertBookModel)
        ...
        call.enqueue(object :
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    insertBookData.postValue(response.body())
    ...
    init {
        CoroutineScope(Dispatchers.IO).launch {
            callWebServiceInsert()
        }
    }
    companion object{
        const val TAG = "INSERT BOOK REPOSITORY";
    }
}